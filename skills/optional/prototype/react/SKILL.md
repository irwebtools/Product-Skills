...
Follow [change-scope](../../../rules/change-scope.md), [fsd-governance](../../../rules/fsd-governance.md), and [react-anti-patterns](../../../rules/react-anti-patterns.md).
Prefer the smallest correct change. Do not redesign unrelated architecture during feature work.

## Architecture Impact Check

Before coding React, identify ownership then location:
...
- Do **not** create new folders unless justified by ownership or demonstrated reuse.
- Do **not** extract code only because a file is large.
- Extract only because the code is reused, has an independent responsibility, or forms a stable boundary.
- Prefer editing the owning `pages/<slice>` (or existing `features/<slice>`) over inventing `src/components`, `src/services`, `src/hooks`, or `src/utils`.
- Touch `shared/` only for infrastructure without business meaning.
- Read `memory/OWNERSHIP.md` before choosing a home.
- Record files expected to change and files explicitly not changed before implementation.

If the ownership boundary is unclear, keep behavior in the page until real reuse appears.
...
Do **not** bypass Feature-Sliced Design to ship a change faster.

- Greenfield Product-Skills React work must follow FSD from the start.
- Existing projects may use an **incremental migration** workflow: keep delivery moving, but each change should move structure toward FSD rather than away from it.
- Prefer small, reviewable migrations (public APIs, layer placement, import direction) over a big-bang rewrite during unrelated feature work.
- The project may migrate gradually, but the **final standard remains FSD-compliant**.
- Do not invent a permanent alternate architecture that replaces FSD as the target.
